...
/** PostgreSQL-backed phone/email + password authentication and recovery. */
export class CredentialService {
  ...
  constructor(
    ...
    this.dummyPasswordHash = hashSecret('invalid-development-password');
  }
  ...
  async authenticatePassword(identifier: string, password: string, context: CredentialRequestContext = {}): Promise<AuthenticatedCredentialUser> {
    return withDatabaseTransaction(
      async (client) => {
        ...
        if (!user) {
          await verifySecret(password, await this.dummyPasswordHash);
        ...
        if (!passwordMatches || user.status !== 'active') {
          if (user.status === 'active') {
            ...
            const shouldLock = nextAttemptCount >= env.AUTH_MAX_FAILED_ATTEMPTS;
            await client.query(
              `UPDATE users
               ...
               WHERE id = $1`,
              [user.id, nextAttemptCount, shouldLock, env.AUTH_LOCKOUT_MINUTES],
            );
            ...
            if (shouldLock) {
              await client.query(
                ...
                [
                  ...
                  JSON.stringify({ failedAttempts: nextAttemptCount, lockoutMinutes: env.AUTH_LOCKOUT_MINUTES }),
                ],
              );
            }
          }
  ...
  async resetPassword(phone: string, code: string, newPassword: string): Promise<void> {
    ...
    try {
      await withDatabaseTransaction(
        async (client) => {
          const userId = await this.otp.verifyOtp(phone, code, 'password_reset', client);
  ...
  async resetPin(phone: string, code: string, newPin: string): Promise<void> {
    const newPasswordHash = await hashSecret(newPin);
    try {
      await withDatabaseTransaction(
        async (client) => {
          const userId = await this.otp.verifyOtp(phone, code, 'pin_reset', client);
          const result = await client.query<{ id: string; status: string }>(
            'SELECT id, status FROM users WHERE id = $1 AND phone = $2 FOR UPDATE',
            [userId, phone],
          );
          const user = result.rows[0];
          if (!user || user.status !== 'active') throw new UnauthorizedError('Unable to reset PIN');
          await client.query(
            `UPDATE users
             SET pin_hash = $2, pin_changed_at = CURRENT_TIMESTAMP,
          ...
          await this.tokens.invalidateAllSessions(userId, client);
          await this.auditCredentialEvent(client, userId, 'auth.pin_reset');
        },
        { isolationLevel: 'READ COMMITTED', maxRetries: 1 },
        this.databasePool,
      );
    } catch (error) {
      if (error instanceof UnauthorizedError) throw new UnauthorizedError('Unable to reset PIN');
      throw error;
    }
  }
