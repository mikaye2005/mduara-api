...
test('BE-03/BE-24 phone + PIN authentication contract', { skip: !databaseUrl, timeout: 120_000 }, async (t) => {
  ...
  async function createActiveUser(pin = '2468') {
    ...
    const user = (await pool.query<{ id: string }>(
      `INSERT INTO users (email, pin_hash, full_name, phone, status)
       ...
       RETURNING id`,
      [`${randomUUID()}@example.test`, pinHash, phone],
    )).rows[0];
    return { id: user.id, phone, pin };
  }

  await t.test('successful PIN login returns the user and issues a verifiable token pair', async () => {
    const user = await createActiveUser();
    assert.deepEqual(await credentials.authenticatePin(user.phone, user.pin), { id: user.id, phone: user.phone });
  ...
  await t.test('wrong PINs are generic and repeated failures lock the account', async () => {
    const user = await createActiveUser();

    for (let attempt = 1; attempt <= 5; attempt += 1) {
      await assert.rejects(
        credentials.authenticatePin(user.phone, '9999'),
    ...
    await assert.rejects(
      credentials.authenticatePin(user.phone, user.pin),
  ...
  await t.test('PIN recovery consumes OTP, clears lockout, revokes sessions and accepts the new PIN', async () => {
    ...
    assert.deepEqual(await credentials.authenticatePin(user.phone, '8642'), { id: user.id, phone: user.phone });
    await assert.rejects(credentials.authenticatePin(user.phone, user.pin), UnauthorizedError);
  });
