...
/** POST /api/v1/auth/register — creates a pending phone+PIN account and sends its activation OTP. */
...
/** POST /api/v1/auth/login — canonical phone + security-PIN sign-in. */
export async function login(req: Request<unknown, unknown, LoginInput>, res: Response): Promise<void> {
  const user = await credentialService.authenticatePin(req.body.phone, req.body.pin, {
    ipAddress: req.ip,
...
/** PATCH /api/v1/auth/pin — verifies the current PIN, replaces it, and invalidates all sessions. */
export async function changePin(req: Request<unknown, unknown, ChangePinInput>, res: Response): Promise<void> {
  await credentialService.changePin(req.user!.id, req.body.currentPin, req.body.newPin);
  sendSuccess(res, { message: 'Security PIN updated. Sign in again on your devices.' });
}
...
export async function resetPin(req: Request<unknown, unknown, ResetPinInput>, res: Response): Promise<void> {
  await credentialService.resetPin(req.body.phone, req.body.code, req.body.newPin);
  sendSuccess(res, { message: 'Security PIN reset successfully. Sign in again on your devices.' });
}
