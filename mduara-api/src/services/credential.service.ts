...
export class CredentialService {
  ...
  async authenticatePin(phone: string, pin: string, context: CredentialRequestContext = {}): Promise<AuthenticatedCredentialUser> {
    return withDatabaseTransaction(
      async (client) => {
        ...
        if (!user) {
          await verifySecret(pin, await this.dummyPinHash);
        ...
        const pinMatches = await verifySecret(pin, user.pin_hash);
        if (!pinMatches || user.status !== 'active') {
  ...
  async changePin(userId: string, currentPin: string, newPin: string): Promise<void> {
    if (currentPin === newPin) throw new BadRequestError('New PIN must be different from the current PIN');
    ...
    await withDatabaseTransaction(
      async (client) => {
        ...
        const user = result.rows[0];
        if (!user || user.status !== 'active' || !(await verifySecret(currentPin, user.pin_hash))) {
        ...
        await this.auditCredentialEvent(client, userId, 'auth.pin_changed');
      },
      { isolationLevel: 'READ COMMITTED', maxRetries: 1 },
      this.databasePool,
    );
